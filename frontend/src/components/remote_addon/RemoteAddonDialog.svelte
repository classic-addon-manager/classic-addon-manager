        ...
        try {
            ...
            if (dependencies.length > 0) {
                ...
                for (const depInfo of dependencies) {
                    ...
                    try {
                        await addons.install(depInfo.manifest);
            ...
            <div class="relative w-full h-48 overflow-hidden shrink-0 rounded-t-lg border-b dark:border-white/10">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-6 pb-4 pt-16 text-white bg-gradient-to-t from-black/60 to-transparent">
                     ...
                     <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm mb-3 text-white/80">
                         ...
                         {#if release}
                             <span class="inline-flex items-center gap-x-1.5">
                                 <Badge variant="secondary" class="inline-flex items-center gap-1 bg-white/20 text-white border-white/30">
                                      <Tag class="w-3 h-3" />
                                      {release.tag_name}
                                 </Badge>
                     ...
                     <div class="flex flex-wrap gap-x-2 gap-y-2 text-sm">
                         ...
                         >
                             <GithubIcon class="w-4 h-4" /> View Code
                         </Button>
                         <Button
                             variant="outline"
                             size="sm"
                         ...
                         >
                             <BugIcon class="w-4 h-4" /> Report Issue
                         </Button>
                     </div>
                </div>
            </div>
                ...
                <div class="flex-1 min-h-0 overflow-y-auto pb-2 pr-2 -mr-2">
                    ...
                    {#if dependencies.length > 0}
                        <Tabs.Content value="dependencies">
                            <p class="mb-4 text-sm text-muted-foreground">This addon requires the following dependencies:</p>
                            <div class="space-y-3">
                                {#each dependencies as d (d.manifest.name)}
                                    ...
                                    >
                                        <div class="flex justify-between items-start mb-1 gap-2">
                                            ...
                                            {#if d.isInstalled}
                                                <Badge variant="secondary" class="text-xs whitespace-nowrap">Installed</Badge>
                                            {:else}
                                                <Badge variant="outline" class="text-xs whitespace-nowrap">Not Installed</Badge>
                                            {/if}
                                        </div>
        ...
        <Dialog.Footer class="p-4 border-t shrink-0">
            <div class="flex justify-between items-center w-full gap-4">
                <div class="flex gap-1 items-center">
                 ...
                 >
                     {#if isInstalled}
                          <Trash2Icon class="w-4 h-4 mr-2"/> Uninstall
                     {:else if !release}
                           Not Available
                     {:else}
                         <DownloadIcon class="w-4 h-4 mr-2"/> Install
                     {/if}
                 </Button>
            </div>
        </Dialog.Footer>
