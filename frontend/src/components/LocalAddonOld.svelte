<script lang="ts">
    ...
    async function handleForceUpdate() {
        ...
        try {
            didInstall = await addons.install(
                await addons.getManifest(addon.name),
            );
        } catch (e: any) {
            if (e.toString().includes("no release found")) {
                dialogErrorMsg = "No release found for this addon.";
            ...
            {:else if latestRelease}
                {#if latestRelease.published_at > addon.updatedAt}
                    <button
                        ...
                        onclickcapture={() => dialogUpdate?.toggle()}
                    >
        ...
        <div class="px-md" style="text-align: left">
            ...
            {:else}
                ...
                <h5 style="margin-top: 0;">
                    Released {formatToLocalTime(latestRelease.published_at)}
                </h5>
                <p>
                    {latestRelease.body ||
                        "No change log provided by the addon"}
                </p>
            {/if}
        </div>
    {/snippet}
            ...
            {:else}
                ...
                {:then found}
                    {#if found}
                        <h3>Good news!</h3>
                        <p>
                            The addon was found in our addon repository. Do you
                            want Classic Addon Manager to manage updates?
                        </p>
                        {#if dialogErrorMsg}
                        ...
                        <button
                            ...
                            class="app-btn app-btn-primary"
                            onclick={handleMatchAddon}>Yes, do it!</button
                        >
                    {/if}
                {/await}
            {/if}
            ...
            <div class="app-flex" style="justify-content: space-between">
                <div>
                    {#if addon.isManaged}
                        ...
                        >
                            <i class="icons10-refresh"></i><span
                                >Force Reinstall</span
                            >
                        </button>
                    {/if}
                    <button
                        class="app-btn app-btn-outline-danger"
                        type="button"
