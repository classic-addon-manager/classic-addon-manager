...
export const Settings = () => {
  ...
  const [pathInput, setPathInput] = useState(aacPath)
  const [errDocsPath, setErrDocsPath] = useState('')

  // Load config on component mount if not already initialized
  useEffect(() => {
    if (!isInitialized) {
      loadConfig()
    }
  ...
  // Sync pathInput with aacPath from store
  useEffect(() => {
    setPathInput(aacPath)
  }, [aacPath])

  const openSelect = async () => {
    try {
      const selectedPath = await ApplicationService.SelectAndValidateDocsPath(
        'Select ArcheAge Classic Documents directory'
      )
                      ...
                      <div className="flex gap-2 p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
                        ...
                        <div className="space-y-1">
                          <div className="text-sm font-medium text-destructive">
                            Error during directory selection.
                          </div>
