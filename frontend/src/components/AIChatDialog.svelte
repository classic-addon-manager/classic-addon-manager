...
<Dialog.Root bind:open>
    <Dialog.Portal>
        ...
        <Dialog.Content class="fixed left-[50%] top-[50%] z-50 w-[calc(100%-2rem)] max-w-[70%] translate-x-[-50%] translate-y-[-50%] gap-0 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-xl overflow-hidden">
            <div class="flex max-h-[calc(100vh-4rem)] h-[650px] lg:h-[80vh] flex-col">
                ...
                <!-- Chat Area -->
                <div bind:this={chatContainer} class="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-thin scrollbar-thumb-secondary scrollbar-track-secondary/20">
                    <div class="h-full">
                        {#if chatHistory.length === 0}
                            <div class="h-full flex flex-col items-center justify-center space-y-6 text-center px-4">
                                <div class="space-y-6 max-h-full">
                                    <img src={supportDaru} alt="Support Daru" class="w-[35vh] h-[35vh] max-w-[280px] max-h-[280px] min-w-[160px] min-h-[160px] object-contain mx-auto drop-shadow-lg hover:scale-105 transition-transform duration-300" />
                                    <div>
                                            ...
                                            {#if message.role === 'user'}
                                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-secondary">
                                                    <Avatar.Root class="h-full w-full">
                                                        <Avatar.Image src={`https://cdn.discordapp.com/avatars/${user.discord_id}/${user.avatar}.png`} alt={user.username} class="h-full w-full object-cover" />
                                                        <Avatar.Fallback class="text-xs">{user.username.substring(0, 2).toUpperCase()}</Avatar.Fallback>
                                                    </Avatar.Root>
                                                </div>
                                            {/if}
                                        </div>
...
<style>
    :global(.chat-message), :global(.chat-message *) { user-select: text !important; }
    ...
    :global(.chat-message.assistant-message h1) { @apply text-xl; }
    :global(.chat-message.assistant-message h2) { @apply text-lg; }
    :global(.chat-message.assistant-message h3) { @apply text-base; }
